...
class HtmlCanvas:
    ...
    def auto_refresh(self, refresh=True):
...
class HtmlServer:
    ...
    def _find_canvas(self, path):
        for canvas in self.canvas_pool:
            if canvas.route == path:
                return canvas
    ...
    def redirect(self, path):
        ...
        try:
            ...
            html = f"""HTTP/1.0 200 OK\r
                ...
                <script>location.href='{path}'</script></head><body>
                <p>Redirecting to {path}...</p></body></html>"""
            client.send(html.encode())
            client.close()
            self.path = path
            return True
    ...
    def _run(self):
        ...
        canvas = self._find_canvas(path)

        if canvas is None:
            canvas = HtmlCanvas("404", 800, 600)
            canvas.draw_text("404 Not Found", x=100, y=100, font_size=30, color="red")
            canvas.set_route("/404")

        if canvas.handle_static_file(req, client, self.static_dir):
            pass
        elif canvas.handle_request(req, client):
            pass
        else:
            html = canvas.render(path)
            client.send("HTTP/1.0 200 OK\r\nContent-type: text/html\r\n\r\n".encode())
            client.send(html.encode())
