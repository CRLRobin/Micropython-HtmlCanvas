...
class HtmlServer:
    ...
    def old_redirect(self, path):
        if path == self.path:
            return True
        try:
            client, _ = self.server.accept()
            redirect_response = f"""HTTP/1.0 302 Found\r
                Location: {path}\r
                Content-type: text/html\r\n\r
                \r\n
                <html><body><script>window.location.href="{path}"</script></body></html>"""
            client.send(redirect_response.encode())
            client.close()
            self.path = path
            return True
        except:
